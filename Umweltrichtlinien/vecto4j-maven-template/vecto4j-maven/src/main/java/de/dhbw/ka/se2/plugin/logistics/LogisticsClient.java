...
public class LogisticsClient {
	public static void main(String[] args) {
		...
		System.out.println(new LogisticsClient().getWeights(vehicle).getMaxPermissibleWeight());
		System.out.println(new LogisticsClient().getDimensions(vehicle).getWidth());
	...
	public VehicleDimensions getDimensions(final VehicleConfiguration vehicle) {
		// Anfrage serialisieren
		ObjectMapper objectMapper = new ObjectMapper();
		objectMapper.registerModule(new JavaTimeModule());
		PipedOutputStream out = new PipedOutputStream();
		...
		// Request vorbereiten
		Request request = Request.post("https://logistics.dh.dtr0cks.de/api/v1/vehicle/dimensions")
				.bodyStream(in)
				.addHeader("Content-Type", "application/json")
				.addHeader("Accept", "application/json");
		...
		try {
			return objectMapper.readValue(body, VehicleDimensions.class);
		} catch (IOException e) {
			throw new RuntimeException("Request failed - cannot parse response!");
		}
	} 	
